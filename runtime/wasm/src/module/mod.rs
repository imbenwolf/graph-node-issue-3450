...
impl<C: Blockchain> WasmInstance<C> {
    ...
    pub fn from_valid_module_with_ctx(
        ...
        link!("ipfs.block", ipfs_block, "host_export_ipfs_block", hash_ptr);
        link!(
            "ipfs.map",
            ipfs_map,
...
impl<C: Blockchain> WasmInstanceContext<C> {
    ...
    /// function ipfs.block(link: String): Bytes
    pub fn ipfs_block(
        &mut self,
        gas: &GasCounter,
        link_ptr: AscPtr<AscString>,
        ...
        if !self.experimental_features.allow_non_deterministic_ipfs {
            return Err(HostExportError::Deterministic(anyhow!(
                "`ipfs.block` is deprecated. Improved support for IPFS will be added in the future"
            )));
        }

        let link = asc_get(self, link_ptr, gas)?;
        let ipfs_res = self.ctx.host_exports.ipfs_block(&self.ctx.logger, link);
        match ipfs_res {
            Ok(bytes) => asc_new(self, &*bytes, gas).map_err(Into::into),

            // Return null in case of error.
            Err(e) => {
                info!(&self.ctx.logger, "Failed ipfs.block, returning `null`";
                                    "link" => asc_get::<String, _, _>(self, link_ptr, gas)?,
                                    "error" => e.to_string());
                Ok(AscPtr::null())
            }
